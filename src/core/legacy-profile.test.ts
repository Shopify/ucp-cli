...
/** Generated by npm releases 0.4.2 … 0.7.0. */
const PROFILE_042_TO_070 = 'profile-0.4.2-to-0.7.0.json'
/** Generated by npm release 0.8.0. */
const PROFILE_080 = 'profile-0.8.0.json'
/** Generated by internal pre-publication 0.1.x builds. */
const PROFILE_PREPUBLIC_01X = 'profile-prepublic-0.1.x.json'
...
function shoppingEntries(body: Record<string, unknown>): Array<Record<string, unknown>> {
  const ucp = body.ucp as Record<string, unknown>
  const services = ucp.services as Record<string, unknown>
  return services['dev.ucp.shopping'] as Array<Record<string, unknown>>
}

function shoppingEntry(body: Record<string, unknown>): Record<string, unknown> {
  const entry = shoppingEntries(body)[0]
  if (entry === undefined) throw new Error('shopping entry missing')
  return entry
}
...
describe('generated-body fingerprints', () => {
  it('matches the frozen fixtures byte-for-byte through canonical JSON', async () => {
    ...
    const expected: Array<[string, string]> = [
      [PROFILE_042_TO_070, '508d145091f0efb805aacd7b21bc738b3dfa108c7b1d59748c66c00fe391b3cd'],
  ...
  it('reports the release each historical body declares', async () => {
    expect(generatedBodyVersion(await fixtureBody(PROFILE_042_TO_070))).toBe('2026-04-08')
    expect(generatedBodyVersion(await fixtureBody(PROFILE_PREPUBLIC_01X))).toBe('2026-04-08')
    expect(generatedBodyVersion(await fixtureBody(PROFILE_080))).toBe('2026-08-25')
  })

  it('is blind to formatting and key order, and sensitive to values', async () => {
    const body = await fixtureBody(PROFILE_080)
...
// ─── ucp-cli 0.4.2–0.7.0 runtime compatibility ────────────────────────────

describe('normalizeProfile042To070', () => {
  it('normalizes the exact generated entry while preserving input and custom declarations', async () => {
    const body = await fixtureBody(PROFILE_042_TO_070)
    const ucp = body.ucp as Record<string, unknown>
    const services = ucp.services as Record<string, unknown>
    const capabilities = ucp.capabilities as Record<string, unknown>
    services['com.acme.loyalty'] = [{ version: '2026-04-08', transport: 'mcp' }]
    capabilities['com.acme.loyalty'] = [
      { version: '2026-04-08', schema: 'https://acme.test/loyalty' },
    ]
    const before = structuredClone(body)

    const normalized = normalizeProfile042To070(body) as typeof body
    const normalizedUcp = normalized.ucp as Record<string, unknown>
    const normalizedServices = normalizedUcp.services as Record<string, unknown>
    const current = JSON.parse(RELEASES['2026-04-08'].agentProfileJson) as typeof body

    expect(normalized).not.toBe(body)
    expect(normalizedUcp).not.toBe(ucp)
    expect(normalizedServices).not.toBe(services)
    expect(normalizedServices['com.acme.loyalty']).toBe(services['com.acme.loyalty'])
    expect(normalizedUcp.capabilities).toBe(capabilities)
    expect(canonicalJson(shoppingEntries(normalized))).toBe(canonicalJson(shoppingEntries(current)))
    expect(body).toStrictEqual(before)
  })

  it.each([
    {
      case: 'pre-publication 0.1.x extra endpoint',
      mutate: (body: Record<string, unknown>) => {
        shoppingEntry(body).endpoint = 'https://example.invalid/agent/no-endpoint'
      },
    },
    {
      case: 'altered entry fields',
      mutate: (body: Record<string, unknown>) => {
        Object.assign(shoppingEntry(body), {
          spec: 'https://example.test/not-stock',
          transport: 'rest',
          schema: 'https://example.test/not-stock.json',
        })
      },
    },
    {
      ...
      mutate: (body: Record<string, unknown>) => {
        delete shoppingEntry(body).spec
      },
    },
    {
      case: 'second shopping entry',
      mutate: (body: Record<string, unknown>) => {
        shoppingEntries(body).push(structuredClone(shoppingEntry(body)))
      },
    },
    {
      case: 'wrong root version',
      mutate: (body: Record<string, unknown>) => {
        const ucp = body.ucp as Record<string, unknown>
    ...
    {
      case: 'wrong service key',
      mutate: (body: Record<string, unknown>) => {
        const services = (body.ucp as Record<string, unknown>).services as Record<string, unknown>
        services['dev.ucp.shopping.other'] = services['dev.ucp.shopping']
        delete services['dev.ucp.shopping']
      },
    },
  ])('returns the original reference for $case', async ({ mutate }) => {
    const body = await fixtureBody(PROFILE_042_TO_070)
    mutate(body)
    expect(normalizeProfile042To070(body)).toBe(body)
  })
})
...
describe('classifyStoredProfile', () => {
  it('reads an untouched generated body with no URL as managed', async () => {
    for (const file of [PROFILE_042_TO_070, PROFILE_080, PROFILE_PREPUBLIC_01X]) {
  ...
  it('keeps a generated body managed under its own release default URL', async () => {
    ...
    expect(
      classifyStoredProfile(await fixtureBody(PROFILE_042_TO_070), {
        profile_url: RELEASES['2026-04-08'].defaultAgentProfileUrl,
      }).kind,
    ).toBe('managed')
    expect(
      classifyStoredProfile(await fixtureBody(PROFILE_080), {
  ...
  it('honors an explicit marker over any fingerprint, in both directions', async () => {
    const stock = await fixtureBody(PROFILE_080)
  ...
  it('re-stamps a kind written without a format_version', async () => {
    ...
    expect(classifyStoredProfile(await fixtureBody(PROFILE_080), { kind: 'diy' })).toEqual({
      kind: 'diy',
      needsMarker: true,
    })
  })
...
describe('readUserProfile — one-time legacy upgrade', () => {
  ...
  it('upgrades an untouched v0.7 profile to managed and leaves profile.json alone', async () => {
    const bytes = await fixtureBytes(PROFILE_042_TO_070)
  ...
  it('upgrades an untouched v0.8 profile to managed', async () => {
    const dir = await seedLegacy('legacy08', await fixtureBytes(PROFILE_080))
  ...
  it.each([
    ...
    const dir = await seedLegacy('unmarked-damage', await fixtureBytes(PROFILE_042_TO_070))
  ...
  it('marks a stock body under a custom URL DIY', async () => {
    await seedLegacy('hosted', await fixtureBytes(PROFILE_080), {
      ...LEGACY_META,
  ...
  it('leaves an explicitly marked DIY stock body DIY, and writes nothing', async () => {
    const dir = await seedLegacy('pinned', await fixtureBytes(PROFILE_080), {
      ...LEGACY_META,
      format_version: PROFILE_FORMAT_VERSION,
      kind: 'diy',
    })
  ...
  it('never touches headers.json', async () => {
    const dir = await seedLegacy('withheaders', await fixtureBytes(PROFILE_042_TO_070))
  ...
  it('is idempotent: the second read neither writes nor changes anything', async () => {
    const dir = await seedLegacy('twice', await fixtureBytes(PROFILE_042_TO_070))
    let writes = 0
  ...
  it('classifies an exploratory scan without stamping metadata or warning on a blocked write', async () => {
    ...
    const dir = await seedLegacy('scanned', await fixtureBytes(PROFILE_042_TO_070))
  ...
  it('keeps working when the marker cannot be written', async () => {
    const warnings: string[] = []
    setWarnWriter((msg) => warnings.push(msg))
    const dir = await seedLegacy('readonly', await fixtureBytes(PROFILE_042_TO_070))
    const untouched = await readFile(join(dir, 'meta.json'), 'utf-8')
