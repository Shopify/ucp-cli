...
- **`result.products[N]`** — each product carries `id`, `title`, `description`, `url`, `categories`, `rating` (product-level aggregate; each variant also carries its own `rating`), `media[]`, `options[]` (variant axes like color/size), `metadata` (ML-inferred attributes, technical specs, top features, unique selling points), `price_range` (`{min, max}` across variants — prefer over `variants[0].price` for buyer-facing display), `variants[M]`.
